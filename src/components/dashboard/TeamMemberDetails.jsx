...
const TeamMemberDetails = () => {
  ...
  // Available team members for reassignment
  const [availableTeamMembers, setAvailableTeamMembers] = useState([]);
  const [reassignTo, setReassignTo] = useState('');
  
  // Available titles for role change
  ...
  useEffect(() => {
    const fetchTeamMemberData = async () => {
      ...
      try {
        ...
        const memberResponse = await fetch(`${API_URL}/team-members/${id}/`, { 
          method: 'GET',
          headers 
        });
        ...
        const assignedTasksResponse = await fetch(`${API_URL}/tasks/?assigned_to=${id}`, { 
          method: 'GET',
          headers 
        });
        
        if (!assignedTasksResponse.ok) {
          throw new Error('Failed to fetch assigned tasks');
        }
        
        const assignedTasksData = await assignedTasksResponse.json();
        
        // Get tasks created by this team member's user
        let createdTasks = [];
        if (memberData.user) {
          const createdTasksResponse = await fetch(`${API_URL}/tasks/?created_by=${memberData.user}`, { 
            method: 'GET',
            headers 
          });
          
          if (createdTasksResponse.ok) {
            createdTasks = await createdTasksResponse.json();
          }
        }
        ...
        for (const task of [...assignedTasks, ...createdTasks]) {
          if (task.history) {
            for (const historyItem of task.history) {
              if (historyItem.actor === memberData.user) {
                historyItems.push({
                  ...historyItem,
                  task_id: task.id,
        ...
        const teamMembersResponse = await fetch(`${API_URL}/team-members/`, { 
          method: 'GET',
          headers 
        });
        ...
        if (teamMembersResponse.ok) {
          ...
          // Filter out the current team member
          setAvailableTeamMembers(teamMembersData.filter(member => member.id !== parseInt(id)));
        }
        
        // Fetch available titles for role change
        ...
        setError(err.message || 'Failed to load team member data. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSuspend = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/team-members/${id}/suspend/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to suspend team member');
      }
      ...
      setActionError(err.message || 'Failed to suspend team member. Please try again.');
    } finally {
  ...
  const handleReassignTasks = async () => {
    if (!reassignTo) {
      setActionError('Please select a team member to reassign tasks to.');
      return;
    }
    ...
    try {
      ...
      if (incompleteTasks.length === 0) {
        setActionError('No incomplete tasks to reassign.');
        setActionLoading(false);
        return;
      }
      ...
      for (const task of incompleteTasks) {
        const response = await fetch(`${API_URL}/tasks/${task.id}/delegate/`, {
          method: 'POST',
          headers,
          body: JSON.stringify({ 
            team_member_id: reassignTo,
            delegation_notes: `Reassigned from ${teamMember.name}`
          })
        });
        
        if (!response.ok) {
          throw new Error(`Failed to reassign task ${task.id}`);
        }
      }
      ...
      setActionError(err.message || 'Failed to reassign some tasks. Please try again.');
    } finally {
      setActionLoading(false);
    }
  };
  
  // Handle changing team member's title/role
  const handleUpdateTitle = async () => {
    if (!newTitle) {
      setActionError('Please select a title.');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to update team member title');
      }
      ...
      setActionError(err.message || 'Failed to update title. Please try again.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleSendPasswordReset = async () => {
    if (!teamMember.email) {
      setActionError('No email address found for this team member.');
      return;
    }
    ...
    try {
      ...
      setActionSuccess('Password reset link sent successfully');
      ...
      setActionError(err.message || 'Failed to send password reset. Please try again.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleRemoveTeamMember = async () => {
    if (!window.confirm(`Are you sure you want to remove ${teamMember.name} from the team? This action cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      // Redirect to team list after successful removal
      setTimeout(() => {
      ...
      setActionError(err.message || 'Failed to remove team member. Please try again.');
    } finally {
      setActionLoading(false);
    }
  };
          ...
          <div className="bg-red-50 border-l-4 border-red-400 p-4">
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2">
                  <a href="/dashboard/team" className="text-sm font-medium text-red-700 hover:text-red-600">
                    Return to Team
                  </a>
                </div>
              </div>
  ...
  const getUserStatusDisplay = () => {
    const status = teamMember.status || (teamMember.user ? 'Active' : 'Pending');
    
    switch(status) {
      case 'Active':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-green-100 text-green-800">Active</span>;
      case 'Suspended':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-red-100 text-red-800">Suspended</span>;
      case 'Pending':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-yellow-100 text-yellow-800">Pending</span>;
      default:
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="flex-1 min-w-0 mb-4 sm:mb-0">
              <div className="flex items-center">
                <div className={`h-16 w-16 rounded-full ${getAvatarBackground(getInitials(teamMember.name)[0])} flex items-center justify-center text-white text-xl font-medium mr-4`}>
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="col-span-2 sm:col-span-1">
                    <div className="text-2xl font-bold text-primary-600">{stats.totalAssigned}</div>
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    <div className="text-2xl font-bold text-primary-600">{stats.averageDaysToComplete}</div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-6">
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{teamMember.name || 'Not set'}</dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{teamMember.last_active_at ? formatDate(teamMember.last_active_at) : 'Not available'}</dd>
                  </div>
                </div>
              </div>
            </div>
                      ...
                      getFilteredTasks().map(task => (
                        <tr key={task.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href={`/dashboard/tasks/${task.id}`} className="text-primary-600 hover:text-primary-900 mr-4">
                              View
                            </a>
                            {!['completed', 'approved', 'rejected'].includes(task.status) && (
                              <a href={`/dashboard/tasks/${task.id}/edit`} className="text-gray-600 hover:text-gray-900">
                                Edit
                              </a>
                            )}
                          </td>
                ...
                <div className="p-6">
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="flex items-center">
                        <div className="text-3xl font-bold text-primary-600">{stats.averageDaysToComplete}</div>
                    ...
                    <li key={index} className="ml-6">
                      ...
                      <h3 className="flex items-center text-sm font-semibold text-gray-900">
                        ...
                        <a 
                          ...
                          className="bg-gray-100 text-primary-600 hover:text-primary-800 text-xs font-medium ml-2 px-2.5 py-0.5 rounded"
                        >
                          {entry.task_title}
                        </a>
                      </h3>
              ...
              <div className="p-6">
                ...
                <div className="mt-4">
                  ...
                  >
                    ...
                    {availableTeamMembers.map(member => (
                      <option key={member.id} value={member.id}>
                        {member.name} ({member.title || 'No title'})
                      </option>
                    ))}
                  </select>
                  ...
                  <button
                    ...
                    disabled={!reassignTo || tasks.assigned.filter(t => !['completed', 'approved', 'rejected'].includes(t.status)).length === 0 || actionLoading}
                  >
                    {actionLoading ? (
                      <>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <button
                      ...
                      disabled={!teamMember.email || actionLoading}
                    >
                      {actionLoading ? (
                        <>
                          <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  ...
                  <div className="pt-6 border-t border-gray-200">
                    <h3 className="text-sm font-medium text-gray-900 mb-2">Change Title</h3>
                    <p className="text-sm text-gray-600 mb-4">
                      Update this team member's title in the organization.
                    </p>
                    <div className="flex items-end space-x-4">
                      ...
                      <button
                        ...
                        disabled={!newTitle || actionLoading}
                      >
                        {actionLoading ? (
                          <>
                            <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
